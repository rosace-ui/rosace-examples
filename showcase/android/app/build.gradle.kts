...
android {
    namespace = "dev.rosace.showcase"
    compileSdk = 34

    defaultConfig {
        applicationId = "dev.rosace.showcase"
        minSdk = 24
        targetSdk = 34
        versionCode = 1
        versionName = "1.0"
...
tasks.register("cargoBuildAndroid") {
    doLast {
        ...
        val src = rootProject.projectDir.parentFile
            .resolve("target/$rustTriple/release/libshowcase.so")
        val destDir = projectDir.resolve("src/main/jniLibs/$abi")
        destDir.mkdirs()
        src.copyTo(destDir.resolve("libshowcase.so"), overwrite = true)
    }
}
