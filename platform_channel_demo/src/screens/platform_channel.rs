            ...
            Column::new()
                ...
                .child(Button::new("Get OS Version").on_press(move || {
                    let result_atom = rosace_ffi::invoke_method(
                        "dev.rosace.platformchanneldemo/device",
                        "getOsVersion",
                        Value::Null,
                    );
...
/// same reason `home_screen`/`counter_screen` take already-created state
/// instead of reading globals themselves.
            ...
            Column::new()
                ...
                .child(Text::new(
                    "A REAL OS permission prompt — this can take as long as \
                     ...
                     ROSACE already (rosace_ffi::request_camera); this demo \
                     app's native code is what actually shows the native \
...
/// `lib.rs::register_platform_channels`) and got a real answer back,
