...
android {
    namespace = "dev.rosace.platformchanneldemo"
    compileSdk = 34

    defaultConfig {
        applicationId = "dev.rosace.platformchanneldemo"
        minSdk = 24
        targetSdk = 34
        versionCode = 1
        versionName = "1.0"
...
tasks.register("cargoBuildAndroid") {
    doLast {
        ...
        val src = rootProject.projectDir.parentFile
            .resolve("target/$rustTriple/release/libplatform_channel_demo.so")
        val destDir = projectDir.resolve("src/main/jniLibs/$abi")
        destDir.mkdirs()
        src.copyTo(destDir.resolve("libplatform_channel_demo.so"), overwrite = true)
    }
}
